...
import { AnimatedWild } from "./AnimatedWild"; // ← استيراد الـ AnimatedWild
...
export function Reel({
  ...
  const ROW_GAP_PX = 20;
  const wrapperHeight = visibleRows * cellSize + (visibleRows - 1) * ROW_GAP_PX;
    ...
    >
      {spinning ? (
        ...
        <div className="absolute inset-0" key={`wild-reel-${wildFxKey}`}>
          {wildWin ? (
            <AnimatedWild trigger={wildFxKey} size={cellSize * 4.5} />
          ) : (
            <img
              src={image1.img}
              alt="IMAGE1"
                    ...
                    <div key={`w-row-${wildFxKey}-${r}`} className="relative">
                      <span
                        style={{
                          ...
                          animation: `stick-in 100ms linear ${base}ms forwards, 
                                      ...
                                      stick-flicker 320ms ease-in-out ${
                                        base + 100
                                      }ms 6 alternate`,
                        }}
                      />
                      <span
                        style={{
                          ...
                          mixBlendMode: "screen",
                          opacity: 0,
                          animation: `stick-in 100ms linear ${
                            base + 120
                          }ms forwards, 
                                      stick-sweep 1040ms ease-in-out ${
                                        base + 120
                                      }ms 2 alternate, 
                                      stick-flicker 320ms ease-in-out ${
                                        base + 220
